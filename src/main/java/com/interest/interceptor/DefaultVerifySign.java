...
public class DefaultVerifySign implements IVerifySign {
    ...
    public boolean verifySign(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            String signHeader = request.getHeader(VerifyConstants.UNICODE_SIGN_KEY);
            ...
            boolean valid = SignUtil.signature(signHeader, param, encodeBodyString);
            ...
            if(timeout.indexOf("ms") != -1){
                checkTime = SignUtil.checkTime(timeStamp, Integer.parseInt(timeout.replace("ms","")));
            }else if(timeout.indexOf("s") != -1){
                checkTime = SignUtil.checkTime(timeStamp, Integer.parseInt(timeout.replace("s","")) * 1000);
            }else{
                //默认300s过期时间
                checkTime = SignUtil.checkTime(timeStamp, 300 * 1000);
            }
            if (!checkTime) {
                //false  不会继续往下执行 不会调用服务接口了 网关直接响应给客户了
